...
export function buildCommerceOpenSourceAdapters(): CommerceOpenSourceAdapter[] {
  return [
    ...
    {
      id: 'opencut-ai',
      name: 'OpenCut-AI editor pattern',
      repositoryUrl: 'https://github.com/Ekaanth/OpenCut-AI',
      useFor: '参考开源 AI 视频编辑器的文字改稿、自动剪辑、时间线复核和本地化编辑体验',
      integrationMode: 'task_manifest',
      customerValue: '把 ChatCut/CapCut 式的轻编辑体验收敛成 Wenai 的“改脚本、重剪前三秒、换封面、再导出”按钮。',
      readiness: 'later',
      guardrail: '只借鉴交互和任务边界；不把未验收编辑器直接暴露给客户，也不让客户处理复杂时间线。',
    },
    {
      id: 'clippedai',
      name: 'ClippedAI long-video clipping pattern',
      repositoryUrl: 'https://github.com/Shaarav4795/ClippedAI',
      useFor: '参考长视频自动找高光、转成短视频、改写标题和生成社媒素材的工作流',
      integrationMode: 'task_manifest',
      customerValue: '客户上传直播回放或测评长素材后，Wenai 先找可用片段，再进电商卖点和标题矩阵。',
      readiness: 'later',
      guardrail: '只处理客户授权素材；高光判断必须回到商品证据、人工抽检和平台发布包。',
    },
    {
      id: 'clipsai',
      name: 'Clips AI library',
      repositoryUrl: 'https://github.com/ClipsAI/clipsai',
      useFor: '参考长视频按转写内容找片段、重构 9:16 画面、聚焦当前说话人和批量导出片段',
      integrationMode: 'task_manifest',
      customerValue: '客户给访谈、直播、讲解或测评长素材时，先得到候选片段和竖屏裁切建议，再进入电商脚本复核。',
      readiness: 'later',
      guardrail: 'Clips AI 适合音频叙事型素材；涉及说话人跟踪和转写 token 时必须做授权、成本和人工复核。',
    },
    {
      id: 'supoclip',
      name: 'SupoClip self-hosted clipping pattern',
      repositoryUrl: 'https://github.com/FujiwaraChoki/SupoClip',
      useFor: '参考自托管短视频生成、转写、亮点片段、字幕和平台适配的完整流水线',
      integrationMode: 'task_manifest',
      customerValue: '把“长素材到短视频”的能力做成客户可审的任务包，而不是让客户自己学剪辑软件。',
      readiness: 'later',
      guardrail: '只借鉴自托管流程和输出结构；模型、转写或外部视频源需要单独授权和成本控制。',
    },
    {
    ...
    },
    {
      ...
      name: 'AI YouTube Shorts Generator pattern',
      repositoryUrl: 'https://github.com/samuraigpt/ai-youtube-shorts-generator',
      useFor: '参考长视频转 Shorts、自动字幕、竖屏裁切、片段选择和发布素材包拆分',
      integrationMode: 'task_manifest',
      customerValue: '电商客户给长视频素材时，先生成多条候选短视频和标题，再由客户选择自发布。',
      readiness: 'later',
      guardrail: '不自动下载或搬运无授权视频；平台发布仍由客户自己登录完成。',
    },
    {
      id: 'buttercut',
      name: 'ButterCut human-in-the-loop editing pattern',
      repositoryUrl: 'https://github.com/barefootford/buttercut',
      useFor: '参考人机协作剪辑、自然语言改片、片段选择和导出复核的轻量工作流',
      integrationMode: 'task_manifest',
      customerValue: '把“AI 帮你剪，但人确认最后版本”的体验用于电商成片抽检和返工。',
      readiness: 'later',
      guardrail: '只借鉴工作流；任何桌面端、Claude Desktop 或本地依赖都必须通过烟测后再进入交付。',
    },
    {
      id: 'yumcut',
      name: 'YumCut short-video studio pattern',
      repositoryUrl: 'https://github.com/IgorShadurin/app.yumcut.com',
      useFor: '参考短视频工作室式的素材、模板、字幕、封面、TTS 和批量导出组织方式',
      integrationMode: 'task_manifest',
      customerValue: '把多功能短视频工作室能力拆成 Wenai 的电商商品项目、模板包和发布包。',
      readiness: 'later',
      guardrail: '先做许可、依赖和商用边界复核；客户界面只暴露 Wenai 可交付结果。',
    },
    {
      id: 'revideo',
...
export function buildCommerceGitHubRemixRadar(
  ...
  const next = adapters
    .filter(adapter => adapter.readiness === 'later' && [
      'vanta-video-engine',
      'openmontage-agent',
      'opencut-ai',
      'clippedai',
    ...
    repoFamilies: [
      {
        id: 'clip-mining',
        label: '长素材切片和高价值片段',
        repoIds: existing(['lossless-cut', 'pyscenedetect', 'auto-editor', 'video-wizard', 'clippedai', 'clipsai', 'supoclip', 'ai-youtube-shorts-generator']),
      ...
      {
        id: 'light-editing',
        label: 'ChatCut 式轻编辑复核',
        repoIds: existing(['opencut-ai', 'buttercut', 'twick-sdk', 'revideo']),
        customerJob: '客户不想学复杂剪辑器，只想改前三秒、换标题、换封面或重剪某一段。',
        adoptNow: '先把轻编辑拆成按钮式任务：改脚本、替换片段、重排镜头、重导出和抽检。',
        keepBehindGate: '把未经烟测的完整编辑器直接嵌到客户界面，或让客户处理复杂轨道。',
        customerOutput: '一键返工任务、改动说明、重新导出的发布包和保留的旧版本。',
      },
      {
        id: 'pipeline-system',
        label: '端到端生产流水线',
        repoIds: existing(['vanta-video-engine', 'openmontage-agent', 'supoclip', 'moneyprinterturbo', 'short-video-maker', 'openshorts-platform', 'yumcut', 'mcp-video']),
