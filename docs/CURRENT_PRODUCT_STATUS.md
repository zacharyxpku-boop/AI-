...
Wenai 现在不是单点 AI 生成 demo，而是电商 AI 内容工业化工作台的可验证内核。当前形态已经覆盖 `Compose / Create / Cut / Cast / Manage` 的首版客户链路：商品资料、模特图任务、开源/本地混剪、稳定渲染队列、多账号标题矩阵、客户自发布包、客服/售后素材、客户回填证据和下一轮复盘。

但它还不是筷子科技级 live platform executor。原因很明确：平台 OAuth、自动发布、广告账户投放、平台数据自动同步、企业云资产权限和规模审计还没有全部接入并跑出证据。生成图、视频、数字人、TTS 等 key 到位后可以增强生产层，但不应阻塞首版交付。

当前进度判断：

| 维度 | 进度 | 判断 |
| --- | ---: | --- |
| 内部全链路骨架 | 89% | 主要模块已具备 API、账本、页面或测试保护，开源混剪、渲染队列、客户审核、Review CRM Handoff Packet、Manual Publish Receipt Board、客服素材和回填复盘都有商用品质门禁 |
| 合作者 POC 展示 | 92% | 可以说明产品形态、能力边界、客户审核/CRM 承接闭环、客户自发布、回填证据门禁、生成 key 增强路径和外部材料路径 |
| 不依赖外部平台的闭环 | 83% | 可做本地/开源/导入式闭环，不假装自动化，并能把客户动作、人工发布回执、渲染任务、回填证据和下一轮复盘拆成验收门禁 |
| 真实商用 readiness | 64% | 可做受控试点；平台 OAuth、广告账户、analytics sync、企业云资产和规模审计是后续自动化门禁，不是首版 P0 |
| 距离筷子科技级平台 | 55% | 核心差距集中在外部平台执行、自动投放/回流、企业资产协作和真实规模证据 |
...
- `Create`：SKU feed、商品卖点、脚本、图片/视频素材、模特图任务、生成 key 接入、交付资产
- `Cut`：AI 视频分析、开源/本地智能混剪、批量版本、稳定渲染队列、成片入库、客户 review token
- `Cast`：多平台账号人设、PubPal/矩阵分发、客户自发布清单、发布证据回填、广告 campaign ledger
...
3. 生成或接入图文/视频资产，进入客户审核。
4. 审核通过后输出账号人设、平台标题、封面、发布时间和客户自发布包。
5. 客户回填链接、截图、CSV 或云盘目录，表现回流到 SKU、素材、账号、campaign 和品牌学习档案。
...
- 客户素材和发布证据：商品图、视频片段、参考链接、发布链接、截图、CSV、评论/客服问题、云盘目录。
- 生成 key：图片、视频、数字人、TTS 等 key 由 owner 提供后增强生成层；未提供时输出 prompt、任务包、本地混剪和客户自发布包。

后续满足规模或正式授权后再接：

- Platform OAuth/account pool pack：抖音/小红书/快手/TikTok/Meta/Google/Amazon/Shopify 的开发者应用、redirect URI、测试账号、发布权限、限频规则。
...
Wenai 现在可以作为合作者评审和受控客户试点的商业 POC core。它可以解释清楚“为什么不等外部 provider 也能先交付”，以及“哪些外部材料只是在规模、自动化或正式授权后再接”，也能避免伪功能宣传。

但在平台 OAuth、campaign、analytics sync、enterprise cloud 和 audited scale evidence 验证前，不建议公开售卖为筷子科技等价平台。当前最准确的定位是：已经具备筷子科技方向的本地优先电商增长工作台和护城河建设路径，可以先以客户自发布和回填复盘进入真实试点。
